...
#pragma warning disable CS0649 // TODO
            ...
            if (orderedScores.Last() != UserScoreItem)
            {
                for (int i = 0; i < orderedScores.Count - 1; i++)
                {
                    ...
                    if (topPosition.Value != bottomPosition.Value - 1)
                    {
                        ...
                        foreach (var score in orderedScores.Skip(maximumScores - 1))
                            ScoresContainer.Remove(score, false); // not sure about false here
