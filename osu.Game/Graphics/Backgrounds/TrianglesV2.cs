            ...
            protected override void Draw(IRenderer renderer)
            {
                ...
                foreach (TriangleParticle particle in parts)
                {
                    ...
                    if (!colour.HasSingleColour)
                    {
                        ...
                        {
                            TopLeft = colour.Interpolate(clamp(triangleQuad.TopLeft)),
                            TopRight = colour.Interpolate(clamp(triangleQuad.TopRight)),
                            BottomLeft = colour.Interpolate(clamp(triangleQuad.BottomLeft)),
                            BottomRight = colour.Interpolate(clamp(triangleQuad.BottomRight)),
                            HasSingleColour = false
                        };
                    }

                    renderer.DrawQuad(texture, drawQuad, colour, new RectangleF(0, 0, 1, 1), textureCoords: textureCoords);
                }

                shader.Unbind();
            }

            private Vector2 clamp(Vector2 vec)
            {
                return vec;
            }
