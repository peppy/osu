...
{
    public class DefaultSkinArgon : Skin
    {
        public static SkinInfo CreateInfo() => new SkinInfo
        {
            ID = osu.Game.Skinning.SkinInfo.DEFAULT_SKIN_ARGON,
            Name = "osu! \"argon\" (2022)",
            Creator = "team osu!",
            Protected = true,
            InstantiationInfo = typeof(DefaultSkinArgon).GetInvariantInstantiationInfo()
        };

        private readonly IStorageResourceProvider resources;

        public DefaultSkinArgon(IStorageResourceProvider resources)
            : this(CreateInfo(), resources)
        {
        }

        [UsedImplicitly(ImplicitUseKindFlags.InstantiatedWithFixedConstructorSignature)]
        public DefaultSkinArgon(SkinInfo skin, IStorageResourceProvider resources)
            : base(skin, resources)
        {
