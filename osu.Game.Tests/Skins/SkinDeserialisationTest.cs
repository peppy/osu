    ...
    public class SkinDeserialisationTest
    {
        ...
        public void TestSkinnableComponentsCoveredByDeserialisationTests()
        {
            ...
            foreach (string oskFile in available_skins)
            {
                ...
                using (var storage = new ZipArchiveReader(stream))
                {
                    ...
                    foreach (var target in skin.TargetInfo)
                    {
                        foreach (var info in target.Value.AllComponents)
        ...
        public void TestDeserialiseModifiedDefault()
        {
            ...
            using (var storage = new ZipArchiveReader(stream))
            {
                ...
                Assert.That(skin.TargetInfo, Has.Count.EqualTo(2));
                Assert.That(skin.TargetInfo[SkinnableTarget.MainHUDComponents], Has.Length.EqualTo(9));
            }
        }
        ...
        public void TestDeserialiseModifiedClassic()
        {
            ...
            using (var storage = new ZipArchiveReader(stream))
            {
                ...
                Assert.That(skin.TargetInfo, Has.Count.EqualTo(2));
                Assert.That(skin.TargetInfo[SkinnableTarget.MainHUDComponents], Has.Length.EqualTo(6));
                Assert.That(skin.TargetInfo[SkinnableTarget.SongSelect], Has.Length.EqualTo(1));

                var skinnableInfo = skin.TargetInfo[SkinnableTarget.SongSelect].AllComponents.First();
            ...
            {
                var skin = new TestSkin(new SkinInfo(), null, storage);
                Assert.That(skin.TargetInfo[SkinnableTarget.MainHUDComponents].AllComponents, Has.Length.EqualTo(8));
                Assert.That(skin.TargetInfo[SkinnableTarget.MainHUDComponents].AllComponents.Select(i => i.Type), Contains.Item(typeof(UnstableRateCounter)));
                Assert.That(skin.TargetInfo[SkinnableTarget.MainHUDComponents].AllComponents.Select(i => i.Type), Contains.Item(typeof(ColourHitErrorMeter)));
                Assert.That(skin.TargetInfo[SkinnableTarget.MainHUDComponents].AllComponents.Select(i => i.Type), Contains.Item(typeof(LegacySongProgress)));
            }
        }
