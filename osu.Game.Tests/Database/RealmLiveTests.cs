...
namespace osu.Game.Tests.Database
{
    public class RealmLiveTests : RealmTest
    {
        ...
        public void TestLiveEquality()
        {
            ...
            {
                ILive<RealmBeatmapInfo> beatmap = realmFactory.CreateContext().Write(r => r.Add(new RealmBeatmapInfo(CreateRuleset(), new RealmBeatmapDifficulty(), new RealmBeatmapMetadata()))).ToLive(realmFactory);

                ILive<RealmBeatmapInfo> beatmap2 = realmFactory.CreateContext().All<RealmBeatmapInfo>().First().ToLive(realmFactory);
        ...
        public void TestAccessAfterStorageMigrate()
        {
            ...
            {
                var beatmap = new RealmBeatmapInfo(CreateRuleset(), new RealmBeatmapDifficulty(), new RealmBeatmapMetadata());

                ILive<RealmBeatmapInfo> liveBeatmap;
        ...
        public void TestScopedReadWithoutContext()
        {
            RunTestWithRealm((realmFactory, _) =>
            {
                ILive<RealmBeatmapInfo>? liveBeatmap = null;
                Task.Factory.StartNew(() =>
                {
                    using (var threadContext = realmFactory.CreateContext())
                    {
                        var beatmap = threadContext.Write(r => r.Add(new RealmBeatmapInfo(CreateRuleset(), new RealmBeatmapDifficulty(), new RealmBeatmapMetadata())));
        ...
        public void TestLiveAssumptions()
        {
            RunTestWithRealm((realmFactory, _) =>
            {
                ...
                {
                    updateThreadContext.All<RealmBeatmapInfo>().QueryAsyncWithNotifications(gotChange);
                    ILive<RealmBeatmapInfo>? liveBeatmap = null;
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        using (var threadContext = realmFactory.CreateContext())
                        {
                            ...
                            var beatmap = threadContext.Write(r => r.Add(new RealmBeatmapInfo(ruleset, new RealmBeatmapDifficulty(), new RealmBeatmapMetadata())));

                            // add a second beatmap to ensure that a full refresh occurs below.
                            // not just a refresh from the resolved Live.
                            threadContext.Write(r => r.Add(new RealmBeatmapInfo(ruleset, new RealmBeatmapDifficulty(), new RealmBeatmapMetadata())));
                    ...
                    Assert.AreEqual(0, updateThreadContext.All<RealmBeatmapInfo>().Count());
                    ...
                    liveBeatmap.PerformRead(resolved =>
                    {
                        ...
                        Assert.AreEqual(2, updateThreadContext.All<RealmBeatmapInfo>().Count());
                ...
                void gotChange(IRealmCollection<RealmBeatmapInfo> sender, ChangeSet changes, Exception error)
                {
                    changesTriggered++;
                }
            });
